...
public class SocialMeliService implements IService {
    ...
    @Autowired
    UsuarioRepository userRepository; //aca nos traemos el repositorio de usuarios

    @Autowired
    PostRepository postRepository;

    ModelMapper mapper = new ModelMapper();
//este seria el constructor, pero no lo uso para poder usar la anotacion ?!
...
//US1
    public void follow(Integer userId, Integer userToFollowId) throws UserNotFoundException, UserAlreadyInUseException, UserSelfUseException {
        ...
        if(userId.equals(userToFollowId)){
            throw new UserSelfUseException(userId); //este Error dice que el usuario se esta siguiendo a si mismo.
        }
